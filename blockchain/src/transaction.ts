...
const validateTxIn = (txIn: TxIn, transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    const referencedUTxOut: UnspentTxOut | undefined = aUnspentTxOuts.find((uTxO) => uTxO.txOutId === txIn.txOutId && uTxO.txOutIndex === txIn.txOutIndex);
...
const getTxInAmount = (txIn: TxIn, aUnspentTxOuts: UnspentTxOut[]): number | null => {
    let findOut: UnspentTxOut | null = findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts);
    if (findOut == null) {
        console.log('error getTxInAmount');
        return null
    } else {
        let findOutAmt = findOut.amount;
        return findOutAmt;
    }
...
const findUnspentTxOut = (transactionId: string, index: number, aUnspentTxOuts: UnspentTxOut[]): UnspentTxOut | null => {
    let found: UnspentTxOut | undefined = aUnspentTxOuts.find((uTxO) => uTxO.txOutId === transactionId && uTxO.txOutIndex === index);

    if (found === undefined) {
        console.log('error findUnspentTxOut');
        return null;
    } else {
        return found;
    }
