...
const validateBlockTransactions = (aTransactions, aUnspentTxOuts, blockIndex) => {
    ...
    const txIns = lodash_1.default(aTransactions)
        .map((tx) => tx.txIns)
        .flatten()
        .value();
...
const getTxInAmount = (txIn, aUnspentTxOuts) => {
    let findOut = findUnspentTxOut(txIn.txOutId, txIn.txOutIndex, aUnspentTxOuts);
    if (findOut == null) {
...
const findUnspentTxOut = (transactionId, index, aUnspentTxOuts) => {
    let found = aUnspentTxOuts.find((uTxO) => uTxO.txOutId === transactionId && uTxO.txOutIndex === index);
    if (found === undefined) {
